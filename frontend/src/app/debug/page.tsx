...
export default function DebugPage() {
  ...
  // Get the first available contract as default
  const defaultContract = Object.keys(contracts)[0] as keyof typeof contracts;
  const [selectedContract, setSelectedContract] = useState<keyof typeof contracts>(defaultContract);
  ...
  // Add null check for contract and abi
  if (!contract || !contract.abi) {
    return (
      <div className="container mx-auto px-4 py-8">
        <Card className="p-6">
          <h2 className="text-lg font-bold mb-4 text-red-600">Error</h2>
          <p>No contract found for: {selectedContract}</p>
        </Card>
      </div>
    );
  }
